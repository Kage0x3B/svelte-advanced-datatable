<!-- @migration-task Error while migrating Svelte code: `<#text>` cannot be a child of `<tr>`. `<tr>` only allows these children: `<th>`, `<td>`, `<style>`, `<script>`, `<template>`. The browser will 'repair' the HTML (by moving, removing, or inserting elements) which breaks Svelte's assumptions about the structure of your components.
https://svelte.dev/e/node_invalid_placement -->
<script lang="ts">
    import SkeletonBadgeWrapper from '$lib/skeleton/SkeletonBadgeWrapper.svelte';
        ...
        {#if item}
            {#each Object.entries(config.columnProperties) as [key, colProp]}
                {#if !colProp.hidden}
                    <td>
                        <InternalDataColumn
                            IconComponent={SkeletonIconWrapper}
                            BadgeComponent={SkeletonBadgeWrapper}
                            {item}
                            {key}
                        />
                    </td>
